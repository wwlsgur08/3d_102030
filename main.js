...
let waitingForFirstDoubleClick = false; // 첫 더블클릭 대기 상태 (실시간 별 추가 시)
...
window.startExploration = function () {
    ...
    setTimeout(() => {
        ...
        // 실시간 별 추가가 아닌 경우에만 온보딩 시작
        if (!waitingForFirstDoubleClick) {
            startOnboarding();
        }
    }, 1000);
...
function closeDetailPanel() {
    ...
    // 첫 번째 더블클릭 후 패널을 닫았다면 온보딩 시작
    if (waitingForFirstDoubleClick) {
        waitingForFirstDoubleClick = false;
        console.log('🎓 첫 별 확인 완료 - 온보딩 시작');
        // 카메라 애니메이션 후 온보딩 시작
        setTimeout(() => {
            startOnboarding();
        }, 1600); // 카메라 이동 완료 후 시작
        return; // 조기 종료하여 기존 온보딩 감지 로직 실행 방지
    }
